...
@HostPlugin(revision = "$Revision: 36558 $", interfaceVersion = 3, names = { "rabidfiles.com" }, urls = { "https?://(www\\.)?rabidfiles\\.com/[A-Za-z0-9]+" })
public class RabidFilesCom extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        if (available_CHECK_OVER_INFO_PAGE) {
            ...
            filename = br.getRegex("Filename\\s*:\\s*</td>\\s*<td(?: class=\"responsiveInfoTable\")?>\\s*([^<>\"]+?)\\s*(?:&[a-zA-Z0-9]+;){0,}<").getMatch(0);
            ...
            filesize = br.getRegex("Filesize\\s*:\\s*</td>\\s*<td(?: class=\"responsiveInfoTable\")?>\\s*([^<>\"]+)\\s*(?:&[a-zA-Z0-9]+;){0,}").getMatch(0);
        } else {
                ...
                for (int i = 1; i <= 5; i++) {
                    ...
                    if (br.containsHTML("data\\-sitekey=")) {
                        ...
                        try {
                            ...
                            if (sm.FAIL_CAUSE_CKEY_MISSING.equals(e.getMessage())) {
