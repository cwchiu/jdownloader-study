...
@HostPlugin(revision = "$Revision: 36886 $", interfaceVersion = 3, names = { "clicknupload.org" }, urls = { "https?://(?:www\\.)?clicknupload\\.(?:com|me|link|org)/(?:vidembed\\-)?[a-z0-9]{12}" })
public class ClickNUploadLink extends antiDDoSForHost {
    ...
    private static final String  COOKIE_HOST                        = "https://clicknupload.org";
    ...
    public String rewriteHost(String host) {
        if (host == null || "clicknupload.com".equals(host) || "clicknupload.me".equals(host) || "clicknupload.link".equals(host) || "clicknupload.org".equals(host)) {
            return "clicknupload.org";
        }
    ...
    public ClickNUploadLink(PluginWrapper wrapper) {
        super(wrapper);
        setConfigElements();
    ...
    private String getFilesizeViaAvailablecheckAlt(final Browser br, final DownloadLink dl) {
        ...
        try {
            postPage(br, "https://" + this.getHost() + "/?op=checkfiles", "op=checkfiles&process=Check+URLs&list=" + Encoding.urlEncode(dl.getDownloadURL()), false);
            filesize = br.getRegex(">" + dl.getDownloadURL() + "</td><td style=\"color:green;\">Found</td><td>([^<>\"]*?)</td>").getMatch(0);
            if (filesize == null) {
                filesize = br.getRegex(this.fuid + "</td><td style=\"color:green;\">Found</td><td>([^<>\"]*?)</td>").getMatch(0);
            }
    ...
    private int getMaxSimultanFreeDls() {
        if (this.getPluginConfig().getBooleanProperty(ENABLE_UNLIMITED_DOWNLOADS, defaultENABLE_UNLIMITED_DOWNLOADS)) {
            ...
            /* 2016-09-19: Tested, 2dls * 2connections = 4connections in total works fine. */
