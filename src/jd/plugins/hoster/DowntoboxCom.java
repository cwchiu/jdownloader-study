...
@HostPlugin(revision = "$Revision: 36859 $", interfaceVersion = 2, names = { "downtobox.com" }, urls = { "https?://(?:www\\.)?downtobox\\.com/(?!streaming/.*?)[A-Za-z0-9]+" })
public class DowntoboxCom extends PluginForHost {

    public DowntoboxCom(PluginWrapper wrapper) {
        super(wrapper);
     ...
     * other: captcha can be skipped via enable_special_stream_handling_download<br />
     */
    ...
    /* Basic constants */
    private final String                   mainpage                                     = "http://downtobox.com";
    private final String                   domains                                      = "(downtobox\\.com)";
    ...
    private static final boolean           enable_special_stream_handling_linkgrabber   = false;
    private static final boolean           enable_special_stream_handling_download      = true;
    /* Known errors */
    ...
    public void correctDownloadLink(final DownloadLink link) {
        ...
        link.setUrlDownload(protocol + "downtobox.com/" + fid);
    }
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        if (available_CHECK_OVER_INFO_PAGE) {
            ...
            if (enable_special_stream_handling_linkgrabber) {
                accessSpecialStreamUrl(fid);
            } else {
            ...
            if (enable_special_stream_handling_linkgrabber) {
                if (br.getURL().contains("/error." + type) || br.getURL().contains("/index." + type) || br.getHttpConnection().getResponseCode() == 404) {
                    throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
                }
    ...
    private void accessSpecialStreamUrl(final String fid) throws IOException {
        /* 2016-06-16: This way we can avoid the captcha!! */
        br.getPage("http://downtobox.com/plugins/mediaplayer/site/_embed.php?u=" + fid);
    }
    ...
    public void doFree(final DownloadLink link, final boolean resume, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (continue_link != null) {
            ...
            if (enable_special_stream_handling_download && available_CHECK_OVER_INFO_PAGE) {
                accessSpecialStreamUrl(fid);
            } else {
                if (available_CHECK_OVER_INFO_PAGE) {
                    br.getPage(link.getDownloadURL());
