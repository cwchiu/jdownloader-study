...
@HostPlugin(revision = "$Revision: 37486 $", interfaceVersion = 3, names = { "clipsage.com" }, urls = { "https?://(?:www\\.)?clipsagedecrypted\\.com/(?:embed\\-)?[a-z0-9]{12}" })
public class ClipsageCom extends antiDDoSForHost {
    ...
    private static final String  COOKIE_HOST                        = "http://clipsage.com";
    private static final String  NICE_HOST                          = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String  NICE_HOSTproperty                  = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String  DOMAINS                            = "(clipsage\\.com)";
    ...
    public ClipsageCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private String[] scanInfo(final String[] fileInfo) {
        ...
        if (SUPPORTS_HTML_FILESIZE_CHECK) {
            if (inValidate(fileInfo[1])) {
                /* 2017-07-27: Special */
                fileInfo[1] = new Regex(correctedBR, "<TD>Filesize</TD>\\s*?<TD>([^<>\"]+)</TD>").getMatch(0);
            }
            if (inValidate(fileInfo[1])) {
    ...
    private void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink == null) {
            /* 2017-07-27: Special 'view' call */
            getPage("/view/" + this.fuid);
