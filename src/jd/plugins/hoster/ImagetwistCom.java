...
@HostPlugin(revision = "$Revision: 36109 $", interfaceVersion = 3, names = { "imagetwist.com" }, urls = { "https?://(?:www\\.)?(?:imagetwist|croea)\\.com/(?:embed\\-)?[a-z0-9]{12}" })
public class ImagetwistCom extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        final String redirect = new Regex(correctedBR, "http\\-equiv=\"Refresh\" content=\"\\d+; URL=(http[^<>\"]+)\"").getMatch(0);
        if (redirect != null) {
            getPage(redirect);
        }
        if (new Regex(correctedBR, "(No such file|>Image Not Found<|>The file was removed by|Reason for deletion:\n|File Not Found|>The file expired)").matches()) {
