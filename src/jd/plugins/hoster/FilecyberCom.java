...
@HostPlugin(revision = "$Revision: 37145 $", interfaceVersion = 3, names = { "filecyber.com" }, urls = { "https?://(?:www\\.)?filecyber\\.com/(?:embed\\-)?[a-z0-9]{12}" })
public class FilecyberCom extends antiDDoSForHost {
     ...
     * mods: waitTime[New RegEx]<br />
     * limit-info: 2017-01-21: premium untested, set FREE account limits<br />
    ...
    @SuppressWarnings("deprecation")
    public FilecyberCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private void waitTime(final DownloadLink downloadLink, final long timeBefore) throws PluginException {
        ...
        if (ttt == null) {
            /* 2017-02-21: New */
            ttt = new Regex(correctedBR, "label\\-danger seconds\">(\\d+)<").getMatch(0);
        }
        if (ttt == null) {
            /* More open RegEx */
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        if (account.getUser() == null || !account.getUser().matches(".+@.+")) {
            throw new AccountInvalidException("You need to use Email as username!");
        }
        try {
            login(account, true);
