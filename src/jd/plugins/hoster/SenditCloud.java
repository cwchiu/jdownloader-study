...
@HostPlugin(revision = "$Revision: 38187 $", interfaceVersion = 3, names = { "sendit.cloud" }, urls = { "https?://(?:www\\.)?sendit\\.cloud/(?:embed\\-)?[a-z0-9]{12}" })
public class SenditCloud extends antiDDoSForHost {
    ...
    private static final String  COOKIE_HOST                        = "http://sendit.cloud";
    private static final String  NICE_HOST                          = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String  NICE_HOSTproperty                  = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String  DOMAINS                            = "(sendit\\.cloud)";
    ...
    public SenditCloud(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, "(No such file|File Not Found|>The file was removed by|Reason for deletion:\n|>The file expired|The file you are)").matches()) {
    ...
    public void handlePremium(final DownloadLink downloadLink, final Account account) throws Exception {
        ...
        if (account.getType() == AccountType.FREE) {
            ...
            logger.info("Final downloadlink = " + dllink + " starting the download...");
            dl = new jd.plugins.BrowserAdapter().openDownload(this.br, downloadLink, dllink, true, -5);
            if (dl.getConnection().getContentType().contains("html")) {
                checkResponseCodeErrors(dl.getConnection());
                logger.warning("The final dllink seems not to be a file!");
