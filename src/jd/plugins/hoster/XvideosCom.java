...
@HostPlugin(revision = "$Revision: 38450 $", interfaceVersion = 2, names = { "xvideos.com" }, urls = { "https?://(www\\.|\\w+\\.)?xvideos\\.com/(video[0-9]+/|embedframe/\\d+|[a-z0-9\\-]+/(upload|pornstar)/[a-z0-9\\-_]+/\\d+/(\\d+)?)" })
public class XvideosCom extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (!dllink.contains(".m3u8")) {
            ...
            final List<M3U8Playlist> playLists = M3U8Playlist.loadM3U8(hlsbest.getDownloadurl(), br2);
    ...
    private String getDllinkHtml5() {
        // hd
        final String dllink = br.getRegex("html5player\\.setVideoUrlHigh\\('(.*?)'\\)").getMatch(0);
        if (dllink == null) {
            // sd
    ...
    public void handleFree(final DownloadLink link) throws Exception {
        ...
        if (!dllink.contains(".m3u8")) {
            ...
            if (hlsbest == null) {
                ...
                final String m3u8 = hlsbest.getDownloadurl();
