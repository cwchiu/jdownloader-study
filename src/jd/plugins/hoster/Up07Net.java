...
@HostPlugin(revision = "$Revision: 35755 $", interfaceVersion = 2, names = { "up07.net" }, urls = { "https?://(www\\.)?up07\\.net/(embed\\-)?[a-z0-9]{12}" })
public class Up07Net extends PluginForHost {
    ...
    private String[] scanInfo(final String[] fileInfo) {
        ...
        if (fileInfo[1] == null) {
            ...
            if (fileInfo[1] == null) {
                fileInfo[1] = new Regex(correctedBR, "<font[^<>]+> file with[^\\(]+\\((.*?)\\)</font>").getMatch(0);
                if (fileInfo[1] == null) {
