...
@HostPlugin(revision = "$Revision: 35960 $", interfaceVersion = 3, names = { "superbshare.com" }, urls = { "https?://(?:www\\.)?superbshare\\.com/file/[A-Za-z0-9]+" })
public class SuperbshareCom extends PluginForHost {
    ...
    private void handleServerErrors() throws PluginException, IOException {
        ...
        br.followConnection();
        if (this.br.getURL().contains("slot-missing")) {
            /* Can happen for free account- and free downloads. */
            throw new PluginException(LinkStatus.ERROR_HOSTER_TEMPORARILY_UNAVAILABLE, "Wait before starting more downloads", 3 * 60 * 1000l);
        }
