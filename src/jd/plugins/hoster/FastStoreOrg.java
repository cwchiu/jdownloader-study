...
@HostPlugin(revision = "$Revision: 37789 $", interfaceVersion = 2, names = { "faststore.org" }, urls = { "https?://(www\\.)?faststore\\.org/(embed\\-)?[a-z0-9]{12}" })
            ...
            for (int i = 0; i <= repeat; i++) {
                ...
                if (correctedBR.contains(";background:#ccc;text-align")) {
                    ...
                    if (letters == null || letters.length == 0) {
                        letters = new Regex(br, "<span style=\"position:absolute;padding\\-left:(\\d+)px;padding\\-top:\\d+px;\">(&#\\d+;)</span>").getMatches();
                        if (letters == null || letters.length == 0) {
