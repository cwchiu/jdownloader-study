...
@HostPlugin(revision = "$Revision: 37353 $", interfaceVersion = 3, names = { "vshare.eu" }, urls = { "https?://(www\\.)?vshare\\.eu/((vid)?embed\\-)?[a-z0-9]{12}" })
@SuppressWarnings("deprecation")
public class VShareEu extends antiDDoSForHost {
    // Site Setters
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        if (cbr.containsHTML("(No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n|<li>The file (expired|deleted by (its owner|administration))|>File is no longer available|>The file you were looking for could not be)")) {
    ...
    public void correctBR() throws Exception {
        ...
        regexStuff.add("(<\\s*(h\\d+)\\s+[^>]*hidden[^>]*>[^<]*<\\s*/\\2[^>]*>)");
