...
@HostPlugin(revision = "$Revision: 38442 $", interfaceVersion = 3, names = { "vidoza.net" }, urls = { "https?://(?:www\\.)?vidoza\\.net/(?:embed\\-)?[a-z0-9]{12}" })
public class VidozaNet extends antiDDoSForHost {
    ...
    private static final String  COOKIE_HOST                        = "http://vidoza.net";
    private static final String  NICE_HOST                          = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String  NICE_HOSTproperty                  = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String  DOMAINS                            = "(vidoza\\.net)";
    ...
    public VidozaNet(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, "(No such file|>The file was (deleted|removed) by|Reason for deletion:\n|File Not Found|>The file expired|'video_wait')").matches()) {
        ...
        if (inValidate(fileInfo[0])) {
            ...
            if (br.containsHTML("<h1></h1>|<title>Watch </title>")) {
                throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
            }
